...
void UIP_CharacterStatWidget::NativeConstruct()
{
	Super::NativeConstruct();

	for (TFieldIterator<FNumericProperty> PropIt(FIP_CharacterStat::StaticStruct()); PropIt; ++PropIt)
	{
...
void UIP_CharacterStatWidget::UpdateStat(const FIP_CharacterStat& BaseStat, const FIP_CharacterStat& ModifierStat)
{
	for (TFieldIterator<FNumericProperty> PropIt(FIP_CharacterStat::StaticStruct()); PropIt; ++PropIt)
	{
		const FName PropKey(PropIt->GetName());
